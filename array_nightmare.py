...
# pcopy = CDWrap(my_copy).compile(use_args=False)
# ncopy = CDWrap(my_copy).compile(use_args=False, jit = njit)

# ccopy = CDWrap(my_copy).add_statics({"array":cuda.local.array, "dtype": "'f8'"}).compile(use_args=False, jit = cjit)
